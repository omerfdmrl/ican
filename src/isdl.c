...
void draw_loss_graph(ISDLContext *context, Iray1D *loss_history, size_t epoch_count, int width, int height) {
    ...
    // Define margins
    int margin = 20;
    int graph_height = height - 2 * margin; // Adjust height for the graph
    ...
    for (size_t i = 1; i < epoch_count; i++) {
        ...
        int x1 = (i - 1) * (width - 2 * margin) / (epoch_count - 1) + margin; // Adjust for margin
        int y1 = height - margin - (loss_history->data[i - 1] / max_loss * graph_height);
        int x2 = i * (width - 2 * margin) / (epoch_count - 1) + margin; // Adjust for margin
...
void draw_model(ISDLContext *context, Model *model, Iray1D *loss_history, size_t epoch) {
    ...
    draw_loss_graph(context, loss_history, epoch, lossWidth, lossHeight);
    for (size_t i = 0; i < model->layer_count; i++) {
        ...
        for (size_t j = 0; j < layerNeuronCount; j++) {
            ...
            SDL_RenderFillCircle(context->renderer, layerX, neuronY, neuronRadius);
