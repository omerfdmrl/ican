...
ISDLContext *isdl_alloc(int width, int height, bool resizable) {
    ...
    ASSERT_MSG(context->window != NULL, "Window could not created");

    context->renderer = SDL_CreateRenderer(context->window, -1, SDL_RENDERER_ACCELERATED);
    ASSERT_MSG(context->renderer != NULL, "Rendere could not created");
...
void img_show(ISDLContext *context, Iray3D *img) {
    ...
    ASSERT_MSG(surface != NULL, "Surface could not created");
    ...
    ASSERT_MSG(texture != NULL, "Texture could not created");
